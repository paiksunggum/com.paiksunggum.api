...
DIP 원칙:
  - 라우터는 구현체(AndrewBlueprintRepository)를 직접 알지 못한다.
...
from apps.titanic.adapter.outbound.repositories.crew_andrew_blueprint_repository import (
...
def get_andrew_blueprint_repository(
    ...
    return AndrewBlueprintRepository(session=db)

def get_andrew_blueprint_use_case(
        repository: AndrewBlueprintPort = Depends(get_andrew_blueprint_repository)
