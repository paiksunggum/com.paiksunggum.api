...
DIP 원칙:
  - 라우터는 구현체(JamesRepository)를 직접 알지 못한다.
...
from apps.titanic.adapter.outbound.repositories.crew_james_repository import JamesRepository
...
def get_james_command_use_case(
        repository: JamesPort = Depends(get_james_command_repository)
) -> JamesCommandUseCase:
