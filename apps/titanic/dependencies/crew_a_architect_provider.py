...
DIP 원칙:
  - 라우터는 구현체(AArchitectRepository)를 직접 알지 못한다.
  - 리턴 타입은 구현체가 아닌 포트(AArchitectUseCase)로 선언한다.
  - 세션은 core 의 get_db 에서 주입받는다 (AsyncSession).
"""
from fastapi import Depends
from core.matrix.oracle_database import get_db
from apps.titanic.adapter.outbound.repositories.crew_a_architect_repository import AArchitectRepository
...
def get_a_architect_repository(
    ...
    return AArchitectRepository(session=db)

def get_a_architect_use_case(
        repository : AArchitectPort = Depends(get_a_architect_repository)
) -> AArchitectUseCase:
