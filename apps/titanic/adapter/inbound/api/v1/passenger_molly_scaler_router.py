...
from apps.titanic.adapter.inbound.api.schemas.passenger_molly_scaler_schema import (
    MollyScalerResponseSchema,
    MollyScalerSchema,
)
from apps.titanic.app.ports.input.passenger_molly_scaler_use_case import MollyScalerUseCase
from apps.titanic.dependencies.passenger_molly_scaler_provider import get_molly_scaler_use_case
...
@molly_scaler_router.get("/myself", response_model=MollyScalerResponseSchema)
async def introduce_myself(
    molly: MollyScalerUseCase = Depends(get_molly_scaler_use_case),
) -> MollyScalerResponseSchema:
    result = await molly.introduce_myself(
        MollyScalerSchema(
            id=10,
            name="Margaret 'Molly' Brown",
        )
    )
    return MollyScalerResponseSchema(id=result.id, name=result.name)
